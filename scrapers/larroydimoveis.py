"""
scrapers/larroydimoveis.py

Scraper for Larroyd Imóveis (https://larroydimoveis.com.br).
...
Key request details (captured from HAR):
  - Header  x-domain: larroydimoveis.com.br   (required — identifies the tenant)
  - Query   filter[transaction]=1             (1 = for sale)
  - Query   custom_query=card                 (returns card-level fields)
  - Query   include=subtype.type,user
  - Query   with_title=true
  - Query   with_grouped_condos=true
  - Query   sort=is_price_shown,calculated_price,id
  - Query   offset=<1-based item offset>
  - Query   limit=<page size>

Note: Unlike other Tecimob-based scrapers, the Larroyd API does NOT use
filter[by_neighborhood_or_city_slug]. All available listings are returned
and city filtering is left to the API layer.

No price/area/bedroom filters are applied at scrape time — all listings
are collected; filtering is the API layer's responsibility.
"""
...
class LarroyImoveisScraper(AgencyScraper):
    """
    Scraper for Larroyd Imóveis.
    ...
    def scrape(self) -> list[Property]:
        """
        Fetches all sale listings from the Larroyd Imóveis API.
    ...
    def _normalize(self, raw: dict[str, Any]) -> Property | None:
        ...
        Raw field reference (from Tecimob API):
          raw["price"]                         "R$300.000"
          ...
          raw["url"]                           slug e.g. "apartamento-a-venda.../558"
