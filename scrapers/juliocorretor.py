...
Scraper para Júlio Teixeira Corretor de Imóveis (https://juliocorretor.com.br).
...
class JulioCorretorScraper(AgencyScraper):
    """
    Scraper para Júlio Teixeira Corretor de Imóveis — Tubarão/SC.
    ...
    def __init__(self, config: AgencyConfig | None = None, client: HttpClient | None = None) -> None:
        ...
        # Injeta os headers necessários para a API Tecimob identificar o domínio
    ...
    def _normalize(self, raw: dict[str, Any]) -> Property | None:
        ...
        try:
            ...
            # Áreas: Prefere total_area, senão usa primary_area ou private_area
            areas = raw.get("areas") or {}
            # Se 'areas' for uma lista vazia [], o 'or {}' garante que não quebra
            if isinstance(areas, list): areas = {}
            
            area_block = areas.get("total_area") or areas.get("primary_area") or areas.get("private_area") or {}
...
def _split_address(formatted: str) -> tuple[str | None, str | None]:
    """Helper para separar 'Bairro - Cidade/UF'."""
