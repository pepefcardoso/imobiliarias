"""
scrapers/citymoveis.py
Scraper para City Imóveis (https://citymoveis.com.br).
Estratégia: API JSON Direta (Plataforma Tecimob api-sites2).
"""
...
# Note: Esta agência usa o endpoint api-sites2 detetado no URL fornecido.
API_ENDPOINT = "https://api-sites2.gerenciarimoveis-cf.com.br/api/properties"
PAGE_SIZE = 21

class CityMoveisScraper(AgencyScraper):
    """
    Scraper para City Imóveis — Tubarão/SC.
    Consome a API JSON da Tecimob.
    """
    name = "citymoveis"
    ...
    def _fetch_page(self, offset: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error("[%s] Erro ao carregar offset %d: %s", self.name, offset, e)
            return None
