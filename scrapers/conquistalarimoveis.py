"""
scrapers/conquistalarimoveis.py
Scraper para Conquistalar Imóveis (https://conquistalarimoveis.com.br).
...
BASE_URL = "https://conquistalarimoveis.com.br"
API_ENDPOINT = "https://api-sites2.gerenciarimoveis-cf.com.br/api/properties"
PAGE_SIZE = 21
CITY_SLUG = "tubarao-sc"

class ConquistalarImoveisScraper(AgencyScraper):
    """
    Scraper para Conquistalar Imóveis — Tubarão/SC.
    Consome a API JSON interna da plataforma Tecimob.
    """
    name = "conquistalarimoveis"

    # O cabeçalho x-domain identifica a agência na infraestrutura da Tecimob
    _HEADERS = {
        "x-domain": "conquistalarimoveis.com.br",
...
def _split_address(formatted: str) -> tuple[str | None, str | None]:
    """Divide o endereço formatado em bairro e cidade."""
    if not formatted:
        return None, None
    if " - " in formatted:
