"""
scrapers/conquistaimoveis.py
Scraper para Imobiliária Conquista (https://conquistaimoveis.com.br).
...
class ConquistaImoveisScraper(AgencyScraper):
    """
    Scraper para Imobiliária Conquista — Tubarão/SC.
    ...
    def _normalize(self, raw: dict[str, Any]) -> Property | None:
        try:
            ...
            # Áreas: A Conquista às vezes envia areas como lista vazia []
            areas = raw.get("areas") or {}
            area_value = None
            if isinstance(areas, dict):
                area_block = areas.get("total_area") or areas.get("primary_area") or {}
                area_value = area_block.get("value")
