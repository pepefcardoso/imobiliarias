"""
scrapers/moradaimoveis.py
Scraper para Morada Imóveis (https://moradaimoveis.com.br).
...
class MoradaImoveisScraper(AgencyScraper):
    """
    Scraper para Morada Imóveis — Tubarão/SC.
    Consome a API JSON interna da plataforma Tecimob.
    """
    name = "moradaimoveis"

    # O cabeçalho x-domain identifica o cliente na API da Tecimob
    _HEADERS = {
    ...
    def _normalize(self, raw: dict[str, Any]) -> Property | None:
        """Mapeia os dados brutos da API Tecimob para o modelo Property."""
        try:
            slug = raw.get("url")
            if not slug: 
                return None
...
def _split_address(formatted: str) -> tuple[str | None, str | None]:
    """Divide a string de endereço (ex: 'Centro - Tubarão/SC') em (bairro, cidade)."""
    if not formatted:
        return None, None
    if " - " in formatted:
