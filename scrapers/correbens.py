"""
scrapers/correbens.py
Scraper para Correbens (https://correbens.com.br).
...
class CorrebensScraper(AgencyScraper):
    """
    Scraper para Correbens — Tubarão/SC.
...
def _split_address(formatted: str) -> tuple[str | None, str | None]:
    """Divide a string de endereço em (bairro, cidade)."""
    if not formatted:
        return None, None
    if " - " in formatted:
